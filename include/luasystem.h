...
namespace Lua
{
	...
	enum class DLLLUA Type : decltype(LUA_TNONE)
	{
		None = LUA_TNONE,
		Nil = LUA_TNIL,
		Bool = LUA_TBOOLEAN,
...
template<class T>
	T Lua::CheckNumber(lua_State *lua,int32_t idx) {return static_cast<T>(CheckNumber(lua,idx));}
...
template<class T>
	void Lua::PushInt(lua_State *lua,T t) {PushInt(lua,static_cast<ptrdiff_t>(t));}
